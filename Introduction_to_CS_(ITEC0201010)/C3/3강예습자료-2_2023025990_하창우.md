...
- **분할-정복**
> 일반적으로 분할-정복은 그대로 해결하기 어려운 큰 문제를 여러개의 작은 문제로 분할한 뒤, 각각 해결하여 결과적으로 큰 문제를 해결하는 방법이다. 프로그램 설계에서의 분할-정복 또한 마찬가지로, 하나의 큰 프로그램을 여러개의 작은 부프로그램(sub-program)으로 나누어 작성한 뒤, 작은 부프로그램을 합쳐 큰 프로그램을 만드는 방식이다.
...
#### **벤치마킹**
먼저 하드웨어에서 밴치마킹이란, 특정 하드웨어의 성능을 평가하는 작업을 의미한다. 이와 유사하게 소프트웨어의 밴치마킹 또한 해당 소프트웨어의 시간에 대한 성능을 평가하는 작업이다. 시스템이 복잡해질수록 단순히 시스템의 세부사항이나 구조만을 보고 성능을 평가하기에는 무리가 있기 때문에 좀더 실제적인(practical) 벤치마킹을 활용한다. [1][15]
